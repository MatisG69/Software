...
import { getMessages, createMessage, markMessageAsRead, getApplicationById, getCandidateApplications } from '@/lib/supabase';
...
export const CandidateMessages = () => {
  ...
  useEffect(() => {
    const loadConversations = async () => {
      ...
      setLoading(true);
      try {
        // Récupérer les applications du candidat pour construire les conversations
        const candidateApplications = await getCandidateApplications(candidate.id);
        const convs = candidateApplications.map(app => ({
          id: app.id,
          jobTitle: app.jobOffer?.title,
          company: app.jobOffer?.company || 'Entreprise',
    ...
    loadConversations();
  }, [candidate?.id, selectedConversation, applicationIdParam]);
  ...
  useEffect(() => {
    const loadMessages = async () => {
      ...
      try {
        ...
        if (candidate?.id) {
          msgs.forEach((msg) => {
            if (!msg.read && msg.receiverId === candidate.id) {
              markMessageAsRead(msg.id);
            }
          });
        }

        // Mettre à jour le dernier message dans les conversations
        if (msgs.length > 0) {
            ...
            prev.map((conv) =>
              conv.id === selectedConversation
                ? {
                    ...
                    unread: msgs.filter((m) => !m.read && m.receiverId === candidate?.id).length,
                  }
                : conv
            )
          );
    ...
    loadMessages();
  }, [selectedConversation, candidate?.id]);

  const handleSendMessage = async () => {
    if (!messageText.trim() || !selectedConversation || !candidate?.id) return;

    setSending(true);
    try {
      // Récupérer l'application pour trouver le receiverId (companyId)
      const application = await getApplicationById(selectedConversation);
      if (!application || !application.companyId) {
        alert('Erreur: impossible de trouver l\'entreprise');
        return;
      }
                ...
                {messages.length === 0 ? (
                  ...
                  messages.map((message) => (
                    <div
                      key={message.id}
                      className={`flex ${
                        message.senderId === candidate?.id ? 'justify-end' : 'justify-start'
                      }`}
                    >
                      <Card
